...
public class DriveTrain extends Subsystem {
	...
	double ticksToInch = (25 + 5/8) / 512;
	double inchToTicks = 512 / (25 + 5/8);
	double pGain = -0.03;
	...
	public void gyroMecanumDrive() {
		...
		try {
			...
			robotDrive4.mecanumDrive_Cartesian((Robot.oi.getDriverJoystick().getRawAxis(0) + 
					(Robot.oi.getDriverJoystick().getRawAxis(2) * -1) + 
					Robot.oi.getDriverJoystick().getRawAxis(3)) * invertDrive,
					Robot.oi.getDriverJoystick().getRawAxis(1) * invertDrive, 
					Robot.oi.getDriverJoystick().getRawAxis(4), ahrs.getAngle() * gyro0);
			//ahrs.getAngle()
		} catch( RuntimeException ex ) {
	...
	public void steamArcadeDrive(double yAxis, double xAxis) {
	...
	public void setArcade(){
		...
		SmartDashboard.putBoolean("Autonomous Drive", changeDrive);
	}
	...
	public void axisStrafeLeft() {
		strafe = Robot.oi.getDriverJoystick().getRawAxis(2);
		frontLeft.set(-1 * strafe * invertDrive);
		frontRight.set(strafe * invertDrive);
		rearLeft.set(strafe * invertDrive);
		rearRight.set(-1 * strafe * invertDrive);
	...
	public void axisStrafeRight() {
		strafe = Robot.oi.getDriverJoystick().getRawAxis(3);
		frontLeft.set(strafe * invertDrive);
		frontRight.set(-1 * strafe * invertDrive);
		rearLeft.set(-1 * strafe * invertDrive);
		rearRight.set(strafe * invertDrive);
	...
	public void pIDMove(double distance) {
		int encoderStart = quadratureEncoder1.get();
		int setpoint = (int) Math.round(distance * inchToTicks) + encoderStart;
		boolean done = false;
		int loopCounter = 0;
		while(!done){
			double pTerm = pGain * (setpoint - (quadratureEncoder1.get() + encoderStart));
			double yValue = Math.max(pIDClipping * -1,  Math.min(pIDClipping, pTerm));
			robotDrive4.arcadeDrive(yValue, 0);
			
			if(Math.abs((quadratureEncoder1.get() + encoderStart) - setpoint) < threshold) {
				loopCounter++;
			} else {
				loopCounter = 0;
			}
